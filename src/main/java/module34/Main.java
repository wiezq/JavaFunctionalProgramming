...
class Main {
    ...
    public static void printFilteredAccounts(List<Account> accounts) {
        Predicate<Account> notEmpty = (account) -> account.balance > 0;
        Predicate<Account> tooMuch = (account) -> !account.locked && account.balance >= 100_000_000;
        List<Account> nonEmptyAccounts = filter(accounts, notEmpty);
        List<Account> accountsWithTooMuchMoney = filter(accounts, tooMuch);
